...
# it), records the environment on `ccr code`, no-ops everything else.
FAKEBIN="$HOME/fakebin"; mkdir -p "$FAKEBIN"
cat > "$FAKEBIN/ccr" <<'EOF'
#!/usr/bin/env bash
case "${1:-}" in
  version) echo "claude-code-router version: 2.0.0"; exit 0 ;;
  code) shift; env | grep -E '^CLAUDE_CODE_MAX_OUTPUT_TOKENS=' > "$REC_ENV_OUT" || true; exit 0 ;;
  *) exit 0 ;;
esac
EOF
chmod +x "$FAKEBIN/ccr"
...
run_native
assert_eq "CLAUDE_CODE_MAX_OUTPUT_TOKENS=131072" "$(cat "$rec_env")" "native path exports the catalog output cap"
...
assert_eq "CLAUDE_CODE_MAX_OUTPUT_TOKENS=131072" "$(cat "$rec_env")" "router path exports the cap (was the 128000-default bug)"
...
guard_line="$(printf '%s\n' "$body" | grep -n '_cma_out_guard' | head -1 | cut -d: -f1)"
split_line="$(printf '%s\n' "$body" | grep -n 'CMA_PROVIDER_TRANSPORT:-native.*== "router"' | head -1 | cut -d: -f1)"
...
printf '%s\n' "$mig_body" | grep -q '_cma_out_guard'; assert_eq 0 $? "regenerated body carries the both-transports cap"
...
assert_eq "CLAUDE_CODE_MAX_OUTPUT_TOKENS=131072" "$(cat "$rec_env")" "cap still exported through the guarded router path"
